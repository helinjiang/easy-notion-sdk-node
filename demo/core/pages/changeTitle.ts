...
(async () => {
  ...
  const res = await pages.extendAPI.changeTitle(NOTION_TOKEN, pageId, `new title ${Math.random().toFixed(5)}`);

  const saveDir = path.join(__dirname, './cache-data');
  fse.ensureDirSync(saveDir);
  fse.writeJsonSync(path.join(saveDir, `changeTitle-${pageId}.json`), res, { spaces: 2 });
})();
